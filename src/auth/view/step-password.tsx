...
import { IApiSendOtp } from '../../types/auth';
...
const PasswordStep = () => {
  ...
  const {mutateAsync: sendOtp, isPending: sendOtpPending} = useMutation({
    mutationKey: ['send-otp-reset-password'],
  ...
  const HandleSendOtp = async (type: string) => {
    if (type === "reset") {
      try {
        await sendOtp({mobileNumber: getParam('mobileNumber'), otpType: 'reset'});
        router.push(
          `${paths.auth.resetPassword}?mobileNumber=${encodeURIComponent(getParam('mobileNumber'))}`
        );
      } catch (e: any) {
      ...
      try {
        await sendOtp({mobileNumber: getParam('mobileNumber'), otpType: 'login'});
        router.push(
          `${paths.auth.otpSignIn}?mobileNumber=${encodeURIComponent(getParam('mobileNumber'))}`
        );
      } catch (e: any) {
        toast.error(e);
      }
