...
export const SignUpSchema = zod.object({
  name: zod
    .string()
    .min(1, {message: 'نام را بدرستی وارد کنید'}),
  mobileNumber: zod
    .string()
  ...
  otp: zod
    .string()
    .min(6, {message: 'کد ارسالی را بدرستی وارد کنید'}),
});
// ------------------------------------------------------------------
const RegisterStep = () => {
  ...
  const {mutateAsync,isPending}=useMutation({mutationKey:['sign-up'],mutationFn:(payload:formData)=>EditCreateRequest(endpoints.AUTH.REGISTER,payload)})
  ...
  const HandleSubmit = handleSubmit(async(data:formData) => {
    try {
