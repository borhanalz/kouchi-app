...
const TicketsCreateDialog = ({dialogStatus}: {dialogStatus:UseBooleanReturn })=>{
  ...
  return <Dialog open={dialogStatus.value} onClose={dialogStatus.onFalse} fullWidth>
    <DialogTitle><Typography variant='h4'>تیکت جدید</Typography></DialogTitle>
    <DialogContent
      sx={{
        p:3
      }}>
      <Form methods={methods}>
        <Stack spacing={2} p={2}>
          ...
          <UploadBox placeholder="فایل مورد نظر را اینجا رها کنید." description="یا برای بارگذاری  اینجا کلیک کنید."/>
        </Stack>
      </Form>
    </DialogContent>
    <DialogActions>
      <Button variant='outlined' onClick={dialogStatus.onFalse} color='error'>لغو</Button>
      <Button variant='contained' color='primary'>ثبت</Button>
    </DialogActions>
  </Dialog>
}
