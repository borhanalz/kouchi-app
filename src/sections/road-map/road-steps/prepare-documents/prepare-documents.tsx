...
const PrepareDocumentsView: FC<IPrepareDocuments> = ({ countryName }) => {
  ...
  const faCountryName = countryLangs
    ?.filter((country) => country.en === countryName)
    .map((selectedCountry) => selectedCountry.fa);
  return (
    <>
      <DashboardContent
        maxWidth={false}
      ...
      >
        <Stack direction="row" justifyContent="left" alignItems="center" spacing={2}>
          <Image
            src={countryName === 'canada' ? canadaImg : italyImg}
            width={60}
            height={30}
            alt="country"
          />
          <Typography fontWeight="bold" variant="h3">
            {' '}
            نقشه راه کارشناسی {faCountryName}
          </Typography>
        </Stack>
        <Stack
          direction="row"
        ...
        >
          <Stack spacing={1} width="100%">
            ...
            <Typography fontSize={10} color="grey" variant="caption">
              تا 8 بهمن - 12 روز
            </Typography>
            <Accordion sx={{ mt: 5, width: '100%' }}>
              ...
              <AccordionDetails>
                <Typography>
                  چون بیشتر از 6 ماه از زمان دریافت مدرکت گذشته باید دوباره اقدام کنی، ایندفعه
                  می‌تونی بیشتر روی اسپیکینگ کار کنی چون روی شانس ویزا شدن تاثیر می‌زاره.
                </Typography>
                <Stack direction="row" spacing={2} mt={3}>
                  <Button
                    variant="contained"
                    onClick={() => {
                      setDialogStatus('addToWorks');
                      dialog.onTrue();
                    }}
                  >
                    اضافه کردن به کارها
      ...
      <Dialog fullWidth open={dialog.value} onClose={dialog.onFalse}>
        <DialogTitle>
          {dialogStatus === 'checkResume' ? 'ارزیابی رزومه' : 'اضافه کردن به کارها'}
        </DialogTitle>
        <DialogContent sx={{ my: 2 }}>
          <Scrollbar>
            {dialogStatus !== 'checkResume' && (
              <Typography>میخوای “آماده کردن مدرک دیپلم” به لیست کار‌هات اضافه بشه ؟ </Typography>
            )}
            {dialogStatus === 'checkResume' && (
              <UploadBox placeholder="فایل رزومه مورد نظر را اینجا رها کنید." />
            )}
          </Scrollbar>
        </DialogContent>
        <DialogActions>
          <Button variant="outlined" onClick={dialog.onFalse} color="primary">
            پس یه بار دیگه نگاه کنم
          </Button>
          <Button variant="contained" color="primary">
            اره دمت گرم
          </Button>
